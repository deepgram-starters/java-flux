...
.PHONY: help check check-prereqs init install install-backend install-frontend build start start-backend start-frontend test update clean status eject-frontend

# Default target: show help
help:
	@echo "Java Flux (Javalin) - Available Commands"
	@echo "========================================="
	@echo ""
	@echo "Setup:"
	...
	@echo ""
	@echo "Development:"
	@echo "  make build             Build the backend shaded JAR"
...
start-backend:
	...
	@if [ ! -f "target/java-flux-1.0.0-shaded.jar" ]; then \
		echo "==> JAR not found, building..."; \
		mvn package -q -DskipTests; \
	fi
	@echo "==> Starting backend on http://localhost:8081"
	java -jar target/java-flux-1.0.0-shaded.jar
